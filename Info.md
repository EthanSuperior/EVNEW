...
- If its a loop already then 0 0 0 with OnEnd set to loop set will run daily w/ checks
...
- RANC: http://asw.forums.cytheraguides.com/topic/16686/
	A counter using N crons and N+1 NCB to store a N bit number [0b1000 would be b4=1]
	In this example b10 is the increment bit.
	- RANC counters are limited to +/- 1 at a time
	- Using an outfit instead of b10 can be used to store multiple +/-s
	NOTE: cron order matters as cron 128 is executed before cron 129
	cronXX1: EnableOn: ( [!b10 !b1 !b2 !b3]=0)		OnStart: ^b4
	cronXX2: EnableOn: ( [!b10 !b1 !b2]=0)			OnStart: ^b3
	cronXX3: EnableOn: ( [!b10 !b1]=0)				OnStart: ^b2
	cronXX4: EnableOn: ( [!b10]=0)					OnStart: ^b1 !b10
- Bi-Directional RANC: http://asw.forums.cytheraguides.com/topic/19025/
	In this example b11 is the decrement bit.
	cronXX1: EnableOn: (( [!b10 !b3 !b2 !b1]=0)|( [!b11 b3 b2 b1]=0))&!(b10&b11)	OnStart: ^b4
	cronXX2: EnableOn: (( [!b10 !b2 !b1]=0)|( [!b11 b2 b1]=0))&!(b10&b11)	 		OnStart: ^b3
	cronXX3: EnableOn: (( [!b10 !b1]=0)|( [!b11 b1]=0))&!(b10&b11)	 				OnStart: ^b2
	cronXX4: EnableOn: (( [!b10]=0)|( [!b11]=0))&!(b10&b11)							OnStart: ^b1 !b10 !b11
	cronXX9: EnableOn: b10 | b11					// Must be last					OnStart: !b10 !b11
...
- Overflow Protection For Max (Inc) // before first bit.
	cronXX0: EnableOn: ( [!b10 !b1 !b2 !b3 !b4]=0)		OnStart: !b10 // Loop on oXXX version&change to OnEnd
- Overflow Protection For Min (Dec)
    cronXX0: EnableOn: ( [!b11 b1 b2 b3 b4]=0)			OnStart: !b11 // Loop on oXXX version&change to OnEnd
