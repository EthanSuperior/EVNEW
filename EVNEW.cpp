...
int CEditor::Init(HINSTANCE hInstance)
{
	...
	iCount = ParseCommandLine(szCommandLine, szArg1);
	szCommandLine += iCount;

	int iShouldLoadAtEnd = 0;

	// The active plugin is initialized via GetCurrentPlugin, which sets its dirty flag to 0.
	GetCurrentPlugin()->m_iIsDirty = 0;

	if (iCount > 0)
	{
		if (strcmp(szArg1, "-torez") == 0)
		{
			iCount = ParseCommandLine(szCommandLine, szArg2);
			szCommandLine += iCount;
			...
			iCount = ParseCommandLine(szCommandLine, szArg3);
			szCommandLine += iCount;
...
int CEditor::Shutdown(void)
{
	...
	if (GetCurrentPlugin()->m_iIsDirty)
	{
		if (AskForSave() == 0)
		{
...
CNovaResource *CEditor::ResourceTemplate(short iID, CNovaResource *pTemplateResource, std::string rezName)
{
	...
	else if (iType == CNR_TYPE_SHIP)
	{
		...
		ResourceTemplate(iID, Workspace::FindById(CNR_TYPE_DESC, tempID), rezName);
...
int CEditor::TempCloseAndSave(void)
{
	...
	if (i == GetCurrentPlugin()->m_vResources[m_iCurrentResourceType].size())
		ListBox_SetCurSel(hwndListResources, -1);
...
BOOL CEditor::MainDialogProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_COMMAND:
	{
		...
		else if (iControlID >= 50000)
		{
			CPlugIn *p;
			if (iControlID < 50100)
				p = Workspace::GetPlugins()[iControlID - 50000];
			else
				p = Workspace::GetData()[iControlID - 50100];
			if (pEditor->GetCurrentPlugin()->m_iIsDirty)
			{
				if (pEditor->AskForSave() == 0)
					return 0;

				pEditor->GetCurrentPlugin()->m_iIsDirty = 0;
			}
